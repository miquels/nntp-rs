...
fn bench_timesources(c: &mut Criterion) {
    ...
    group.bench_function("time::now_utc()", |b| b.iter(|| {
        time::now_utc();
    }));
    group.bench_function("libc::CLOCK_REALTIME", |b| b.iter(|| {
        let mut ts = libc::timespec{ tv_sec: 0, tv_nsec: 0 };
        unsafe { libc::clock_gettime(libc::CLOCK_REALTIME, &mut ts); }
    }));
    group.bench_function("libc::CLOCK_REALTIME_COARSE", |b| b.iter(|| {
        let mut ts = libc::timespec{ tv_sec: 0, tv_nsec: 0 };
        unsafe { libc::clock_gettime(libc::CLOCK_REALTIME_COARSE, &mut ts); }
    }));
    group.bench_function("libc::CLOCK_MONOTONIC", |b| b.iter(|| {
        let mut ts = libc::timespec{ tv_sec: 0, tv_nsec: 0 };
        unsafe { libc::clock_gettime(libc::CLOCK_MONOTONIC, &mut ts); }
    }));
    group.bench_function("libc::CLOCK_MONOTONIC_COARSE", |b| b.iter(|| {
        let mut ts = libc::timespec{ tv_sec: 0, tv_nsec: 0 };
        unsafe { libc::clock_gettime(libc::CLOCK_MONOTONIC_COARSE, &mut ts); }
    }));
    group.bench_function("libc::gettimeofday", |b| b.iter(|| {
